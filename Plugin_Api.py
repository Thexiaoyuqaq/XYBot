...
async def Plugins_Request(message, plugins):
    # 处理消息的逻辑
    for plugin_name, plugin in plugins:
        if hasattr(plugin, "Requst") and callable(getattr(plugin, "Requst")):
            try:
                await plugin.Requst(message)
            except Exception as e:
                curr_time = datetime.datetime.now()
                time_str = datetime.datetime.strftime(curr_time, '%H:%M:%S')
                event_Time = "[" + time_str + "]"
            ...
            time_str = datetime.datetime.strftime(curr_time, '%H:%M:%S')
            event_Time = "[" + time_str + "]"
            #print(event_Time + f"[警告][插件][跳过][事件][进群][{plugin_name}] 插件缺少 'Requst' 方法，跳过执行。")
