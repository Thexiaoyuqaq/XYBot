...
async def set_group_card(event_message_Group_ID,card,user_id):#设置群员名片
    url = f"http://{host}:{http_port}/set_group_card?group_id={event_message_Group_ID}&user_id={user_id}&card={card}" #&r
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
            return rsjson
async def set_group_ban(event_message_Group_ID,user_id,time):#设置群员名片
    if user_id != 3443135327:
        url = f"http://{host}:{http_port}/set_group_ban?group_id={event_message_Group_ID}&user_id={user_id}&duration={time}" #&r
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                rsjson = await response.json()
                return rsjson
