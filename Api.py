...
async def get_group_info(event_message_Group_ID):#获取群信息API
    url = f"http://localhost:25010/get_group_info?group_id={event_message_Group_ID}&no_cache=true" 
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
            return rsjson
async def set_group_card(event_message_Group_ID,card,user_id):#设置群员名片
    url = f"http://localhost:25010/set_group_card?group_id={event_message_Group_ID}&user_id={user_id}&card={card}" #&r
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
            return rsjson
async def set_group_ban(event_message_Group_ID,user_id,time):#设置群员名片
    if user_id != 3443135327:
        url = f"http://localhost:25010/set_group_ban?group_id={event_message_Group_ID}&user_id={user_id}&duration={time}" #&r
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                rsjson = await response.json()
                return rsjson
async def get_group_member_info(event_message_Group_ID,userid):#获取群成员信息API
    url = f"http://localhost:25010/get_group_member_info?group_id={event_message_Group_ID}&user_id={userid}&no_cache=true"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
            return rsjson
async def send_group_forward_msg(event_message_Group_ID,event_message_Message):#发送合并消息
    url = f"http://localhost:25010/send_group_forward_msg?group_id={event_message_Group_ID}&messages={event_message_Message}&no_cache=true"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
            return rsjson
async def send_Groupmessage(event_message_Group_ID,event_message_Message_ID, event_message_Message,awa):#发送群消息API
    if awa == True:
        url = f"http://localhost:25010/send_group_msg?group_id={event_message_Group_ID}&message=[CQ:reply,id=" + str(event_message_Message_ID) + "]" + event_message_Message
    if awa == False:
         url = f"http://localhost:25010/send_group_msg?group_id={event_message_Group_ID}&message={event_message_Message}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
...
async def set_GroupRequest(flag,type):#同意加群操作API
    url = f"http://localhost:25010/set_group_add_request?approve=true&type={type}&flag={flag}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
            return rsjson
async def set_FreindRequest(flag):#同意好友操作API
    url = f"http://localhost:25010/set_friend_add_request?approve=true&flag={flag}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
            return rsjson
async def delete_msg(message_id):#撤回消息API
    url = f"http://localhost:25010/delete_msg?message_id={message_id}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            rsjson = await response.json()
            return rsjson