...
async def cmd_Log(event_Post_Type, event_original):
    ...
    if event_Post_Type == "message":
        ...
        if event_Message_From == "group":
            ...
            logger.info(message="[信息][消息][接收][群聊][" + str(event_message_Group_Name) + "](" + str(
                event_message_Group_ID) + ") [" + str(event_message_Self_Role) + "]" + str(
                event_message_Self_Name) + "(" + str(event_message_Self_ID) + ")" + ": " + str(
                event_message_Messages) + "   (" + str(event_message_Message_ID) + ")")
        if event_Message_From == "private":
            event_message_Self_Name = event_original["sender"]["nickname"]
            event_message_Messages = event_original["message"]
            event_message_Self_ID = event_original["user_id"]
            logger.info(
                message="[信息][消息][接收][好友] " + event_message_Self_Name + "(" + str(event_message_Self_ID) + ") : " + str(
                    event_message_Messages))
